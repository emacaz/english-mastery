...
export function FAQCard({ question }) {
    ...
    return (
        <div className="my-11 relative">
            ...
            >&#8595;</span>
            <h2 className="text-xl text-center font-semibold mb-3">{question.question}</h2>
            <p className={`text-base text-justify my-5 ${isAnswerVisible ? 'block' : 'hidden'}`}>{question.answer}</p>
            <hr />
        </div>
    );
}
